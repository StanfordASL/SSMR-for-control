...
SETTINGS = {
    ...
    'poly_u_order': 2,
    'input_training_traj': "open-loop",
    'input_train_ratio': 0.8,
    'input_test_traj': ["open-loop_circle", ]
}
...
if not models:
    model_save_dir = join(model_dir, "model_001")
else:
    model_save_dir = join(model_dir, f"model_{int(models[-1].split('_')[-1])+1:03}")
...
Data = {}
Data['oData'] = utils.import_pos_data(join(SETTINGS['robot_dir'], SETTINGS['data_dir'], SETTINGS['decay_dir']),
...
if SETTINGS['observables'] == "pos-vel":
    ...
    raise RuntimeError("Unknown type of observables, should be ['delay-embedding', 'pos-vel']")
# training data
(t, z), u = utils.import_pos_data(data_dir=join(SETTINGS['robot_dir'], SETTINGS['data_dir'], SETTINGS['input_training_traj']),
                                  rest_file=join(SETTINGS['robot_dir'], SETTINGS['rest_file']),
...
# ====== regress B matrix ====== #
assemble_features = lambda u, x: lambda u, x: utils.multivariate_polynomial(u, order=SETTINGS['poly_u_order']) # utils.multivariate_polynomial(np.vstack([u, x]), order=SETTINGS['poly_u_order']) # 
...
for test_traj in SETTINGS['input_test_traj']:
    traj_dir = join(SETTINGS['robot_dir'], SETTINGS['data_dir'], test_traj)
