...
from florianworld.utils import get_quantized_statistics_along_axis
...
class PlotContext:
    ...
    def __init__(self, filename=None, subplot_rows=1, subplot_cols=1, base_width_inch=10, base_height_inch=7,
                 paper_style_width=None, paper_style_height=None):
        ...
        if use_paper_style_plots:
            ...
            # self.height_inch = 3
            self.width_inch = 6 if paper_style_width is None else paper_style_width
            self.height_inch = 3 if paper_style_height is None else paper_style_height
            # self.height_inch = 3
            self.FORMATS = [".pdf"]
...
def time_series_plot(pc: PlotContext, time, data, labels, title="", ylabel=None, use_scatter=False, use_log=False,
                     ...
                     xlim=None, axis_equal=False, dashed=None):
    ...
    for i in range(len(data)):
        ...
        linestyle = '-'
        if dashed is not None and ((isinstance(dashed, list) and dashed[i]) or
                                   (not isinstance(dashed, list) and dashed)):
            linestyle='--'

        if isinstance(dashed, list) and len(dashed) > 2 and i == 2:
            linestyle='-.'

        if use_scatter:
            ax.scatter(t, data[i], label=label, color=DEFAULT_COLORS[i])
        else:
            ax.plot(t, data[i], label=label, color=DEFAULT_COLORS[i], linestyle=linestyle)
